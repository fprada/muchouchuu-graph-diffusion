...
def fit_ds(times: np.ndarray, probs: np.ndarray, width: int) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    for i in range(n):
        ...
        # Strict-centred estimator: unsupported edge locations
        # remain NaN and are excluded from crossing/plateau inference.
        if idx.size != width:
            continue
...
def main() -> None:
    ...
    late_mask = (
        (times >= args.plateau_tmin)
        & np.isfinite(central_ds)
    )
    n_late = int(np.sum(late_mask))
    if n_late < 3:
        raise ValueError(
            f"--plateau-tmin={args.plateau_tmin} leaves only {n_late} "
            "late-time points; at least 3 are required"
        )

    late_log_times = np.log(times[late_mask])
    plateau_samples = np.mean(boot_ds[:, late_mask], axis=1)
    ...
    central_plateau = float(np.mean(central_ds[late_mask]))
    central_slope = float(
        np.polyfit(late_log_times, central_ds[late_mask], 1)[0]
    )
