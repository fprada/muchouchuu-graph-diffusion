...
def local_window_indices(n: int, center: int, width: int) -> np.ndarray:
    ...
    if width > n:
        raise ValueError(f"--window-points={width} exceeds number of times={n}")
    half = width // 2
    lo = center - half
    hi = center + half + 1
    if lo < 0:
...
def fit_ds(times: np.ndarray, probs: np.ndarray, width: int) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Return centered-window d_s, R^2, and relative RMS residual in log-probability."""
    n = len(times)
    ...
    # Only genuinely centered windows.
    for i in range(half, n - half):
        idx = np.arange(i - half, i + half + 1, dtype=np.int64)
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        default=3584.0,
        help=(
            "Minimum diffusion time used for the late-time plateau and "
            "residual-slope bootstrap analysis"
        ),
    )
    args = ap.parse_args()
    ...
    # Save complete bootstrap spectral-dimension trajectories. This preserves
    # correlations among overlapping sliding windows for the late-time analysis.
    np.save(
        outdir / "bootstrap_spectral_dimension_curves.npy",
        boot_ds,
    )
    ...
    # Crossing distribution with continuous log-time interpolation.
    ...
    # Late-time plateau and residual-slope bootstrap analysis
    late_mask = (times >= args.plateau_tmin) & np.isfinite(central_ds)
    n_late = int(np.sum(late_mask))
    if n_late < 3:
        print(
           f"WARNING: --plateau-tmin={args.plateau_tmin} leaves only "
           f"{n_late} valid centered late-time points; "
           "plateau/slope statistics will be set to NaN."
        )

    if n_late >= 3:
        late_log_times = np.log(times[late_mask])

        plateau_samples = np.mean(
            boot_ds[:, late_mask],
            axis=1,
        )

        slope_samples = np.empty(args.bootstrap, dtype=float)

        for b in range(args.bootstrap):
            slope_samples[b] = np.polyfit(
                late_log_times,
                boot_ds[b, late_mask],
                1,
            )[0]

        central_plateau = float(
            np.mean(central_ds[late_mask])
        )

        central_slope = float(
            np.polyfit(
                late_log_times,
                central_ds[late_mask],
                1,
            )[0]
        )

    else:
        plateau_samples = np.full(
            args.bootstrap,
            np.nan,
            dtype=float,
        )

        slope_samples = np.full(
            args.bootstrap,
            np.nan,
            dtype=float,
        )

        central_plateau = np.nan
        central_slope = np.nan

    def bootstrap_quantiles(values: np.ndarray) -> dict[str, float]:
    ...
    plateau_quantiles = bootstrap_quantiles(plateau_samples)
    slope_quantiles = bootstrap_quantiles(slope_samples)

    plateau_summary = {
        "late_time_definition": (
            f"diffusion_time >= {float(args.plateau_tmin)}"
        ),
        "plateau_tmin": float(args.plateau_tmin),
        "number_of_late_points": n_late,
        "late_diffusion_times": [
            float(v) for v in times[late_mask]
        ],
        "central_plateau_mean": central_plateau,
        "central_slope_d_ds_d_log_t": central_slope,
        "bootstrap_plateau_quantiles": plateau_quantiles,
        "bootstrap_slope_quantiles": slope_quantiles,
        "probability_plateau_ge_3": float(
            np.mean(plateau_samples >= 3.0)
        ),
        "probability_plateau_lt_3": float(
            np.mean(plateau_samples < 3.0)
        ),
        "probability_slope_gt_0": float(
            np.mean(slope_samples > 0.0)
        ),
        "probability_slope_lt_0": float(
            np.mean(slope_samples < 0.0)
        ),
        "interpretation_note": (
            "Each bootstrap plateau and slope is computed from one complete "
            "resampled probe trajectory, preserving correlations among "
            "overlapping sliding windows."
        ),
    }

    np.save(
        outdir / "late_time_plateau_samples.npy",
        plateau_samples,
    )
    np.save(
        outdir / "late_time_slope_samples.npy",
        slope_samples,
    )
    pd.DataFrame({
        "bootstrap_index": np.arange(args.bootstrap),
        "late_time_plateau_ds": plateau_samples,
        "slope_d_ds_d_log_t": slope_samples,
    }).to_csv(
        outdir / "late_time_plateau_bootstrap_samples.csv",
        index=False,
    )
    (outdir / "late_time_plateau_summary.json").write_text(
        json.dumps(plateau_summary, indent=2) + "\n"
    )

    result = pd.DataFrame({
        "diffusion_time": times.astype(int),
        "window_points": np.full(len(times), args.window_points, dtype=int),
    ...
    # Preserve the original central hard gates: CI95 contains 3 and R^2 passes.
    ...
    metadata = {
        ...
        "late_time_plateau_tmin": float(args.plateau_tmin),
        "late_time_plateau_summary": str(
            (outdir / "late_time_plateau_summary.json").resolve()
        ),
    ...
    print("Wrote:", outdir / "bootstrap_spectral_dimension_curves.npy")
    print("Wrote:", outdir / "late_time_plateau_samples.npy")
    print("Wrote:", outdir / "late_time_slope_samples.npy")
    print("Wrote:", outdir / "late_time_plateau_bootstrap_samples.csv")
    print("Wrote:", outdir / "late_time_plateau_summary.json")
    ...
    print("\nLate-time plateau analysis")
    print("Plateau t_min:", args.plateau_tmin)
    print("Number of late-time points:", n_late)
    print("Central plateau mean:", central_plateau)
    print("Central slope d(d_s)/d(log t):", central_slope)
    print("Bootstrap plateau quantiles:", plateau_quantiles)
    print("Bootstrap slope quantiles:", slope_quantiles)
    print("P(plateau >= 3):", plateau_summary["probability_plateau_ge_3"])
    print("P(slope > 0):", plateau_summary["probability_slope_gt_0"])
