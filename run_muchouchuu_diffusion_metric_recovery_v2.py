#!/usr/bin/env python3
"""Safe MuchoUchuu full-box diffusion sketch and Euclidean metric recovery.
...
def main(a):
    ...
    xyz_path = Path(a.xyz_binary).expanduser().resolve()
    graph_path = Path(a.graph).expanduser().resolve()
    ...
    byte_count = xyz_path.stat().st_size
    if byte_count % bytes_per_row != 0:
        raise ValueError(
            'XYZ file size is not divisible by 12 bytes per float32 row.'
        )
    ...
    if a.n is None:
        ...
        raise ValueError(
            '--n={} but XYZ file size implies N={}'.format(
                a.n, inferred_n
            )
        )
    ...
    if getattr(a, 'coordinate_metadata', None):
        coordinate_metadata_path = Path(
            a.coordinate_metadata
        ).expanduser().resolve()
        coordinate_metadata = json.loads(
            coordinate_metadata_path.read_text()
        )
        if coordinate_metadata.get('written_objects') != a.n:
            raise ValueError(
                'Coordinate metadata written_objects does not match N.'
            )

    if getattr(a, 'graph_metadata', None):
        graph_metadata_path = Path(
            a.graph_metadata
        ).expanduser().resolve()
        graph_metadata = json.loads(
            graph_metadata_path.read_text()
        )
        if graph_metadata.get('n') != a.n:
            raise ValueError(
                'Graph metadata n does not match coordinate N.'
            )
        if float(graph_metadata.get('box_size_mpc_h')) != float(a.box_size):
            raise ValueError(
                'Graph metadata box size does not match --box-size.'
            )

    print('MuchoUchuu diffusion metric-recovery run', flush=True)
    print('N={:,}'.format(a.n), flush=True)
    print('Box size={} h^-1 Mpc'.format(a.box_size), flush=True)
    print('Sketch dimension={}'.format(a.sketch_dim), flush=True)
    print('Diffusion times={}'.format(sorted(set(a.times))), flush=True)
    ...
    meta.update(
        ...
        resume_checkpoint_sha256=checksum(checkpoint_path),
        xyz_binary=str(xyz_path),
        xyz_binary_bytes=byte_count,
        inferred_n=inferred_n,
        graph=str(graph_path),
        diffusion_operator='D^{-1} A',
    ...
    # If resuming exactly at a requested diagnostic time, evaluate the
    # already-loaded checkpoint without advancing the diffusion.
    if start in times and start not in done:
        dr,cr=diagnostics(
            graph_name,start,src,i,j,physical,
            a.fit_windows,a.fit_bins,
            a.curve_distance_min,a.curve_distance_max,
            a.pair_distance_bins
        )
        append_rows(diag_path,df,dr)
        append_rows(curve_path,cf,cr)
        done.add(start)
        print(
            'Wrote diagnostics for resumed checkpoint t={}'.format(start),
            flush=True
        )
...
def parse_args():
    p = argparse.ArgumentParser(
        description=(
            'Run the MuchoUchuu full-box random-walk diffusion sketch '
            'and Euclidean metric-recovery diagnostics.'
        )
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help='Optional halo count; inferred from XYZ file size by default.',
    )
    p.add_argument('--box-size',type=float,default=6000.0); p.add_argument('--output-dir',default='muchouchuu_knn_diffusion_metric_recovery')
    ...
    p.add_argument('--pair-distance-bins',type=int,default=48); p.add_argument('--pairs-per-bin',type=int,default=500); p.add_argument('--curve-distance-min',type=float,default=50.0); p.add_argument('--curve-distance-max',type=float,default=1200.0); p.add_argument('--max-pair-rounds',type=int,default=3000)
    p.add_argument('--fit-windows',nargs='+',type=float,default=[75,150,150,300,300,600,600,1200])
    p.add_argument('--fit-bins',type=int,default=16)
    p.add_argument('--seed',type=int,default=12345)
    p.add_argument(
        '--coordinate-metadata',
        default=None,
        help='Optional selector metadata JSON for consistency checks.',
    )
    p.add_argument(
        '--graph-metadata',
        default=None,
        help='Optional graph parameters.json for consistency checks.',
    )
    a=p.parse_args()
