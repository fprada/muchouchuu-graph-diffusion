...
using the final MuchoUchuu calibration
    ...
    A_0.5 is fitted directly from the Step-7 50% containment scale.
...
def main(args) -> None:
    ...
    ax1.plot(
        t, model, "--", lw=1.4,
        label=rf"Fit: $\ell_{{0.5}}(t)=A_{{0.5}}\sqrt{{t}}$, $A_{{0.5}}={prefactor:.4f}$",
    )
    ...
    ax2.set_ylabel(r"$\ell_{0.5}(t)/(A_{0.5}\sqrt{t})$")
    ax2.set_title(r"(b) Compensated $\sqrt{t}$ scaling", loc="left", fontweight="bold")
    ax2.grid(True, alpha=0.22)

    fig.suptitle(
        "MuchoUchuu real-space diffusion containment scaling",
        fontsize=16,
        fontweight="bold",
        y=0.965,
    )
    fig.text(
        0.5,
        0.91,
        r"The real-space containment scale follows $\ell_{0.5}(t)=A_{0.5}\sqrt{t}$ over the fitted interval.",
        ha="center",
        va="center",
        fontsize=10.8,
    )
    ...
    note = (
        rf"Independent fit to the 50% containment scale: "
        rf"$A_{{0.5}}={fitted_prefactor:.6f}\ "
        rf"h^{{-1}}\mathrm{{Mpc}}\,\mathrm{{step}}^{{-1/2}}$; "
        rf"fractional RMS about the fitted $\sqrt{{t}}$ relation "
        rf"= {rms_fractional:.3%}."
    )
    fig.text(
        0.11, 0.045, note,
    ...
    diagnostics = pd.DataFrame([{
        ...
        "fitted_containment_prefactor_mpc_h_step_minus_half": fitted_prefactor,
    ...
    print(f"Fitted 50% containment prefactor A_0.5 = {fitted_prefactor:.12f} h^-1 Mpc step^-1/2")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", default="appendix_figureA1_muchouchuu_diffusion_length_v3.pdf")
    parser.add_argument("--png", default="appendix_figureA1_muchouchuu_diffusion_length_v3.png")
    parser.add_argument("--diagnostics-csv", default="appendix_figureA1_muchouchuu_diffusion_length_v3_diagnostics.csv")
